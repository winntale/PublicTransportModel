...
public:
	...
	property System::String^ color {
		...
		void set(System::String^ _value) {
			if (System::String::IsNullOrEmpty(_value)) {
				MessageBox::Show("¬ы не указали цвет машины такси", "ќшибка",
	...
	// метод описани€ движени€ и поворота в 0 состо€нии машины (завершЄн)
	void Move(array<array<Point^>^>^ Vertices) {
		// в зависимости от направлени€ (direction) мен€ютс€ координаты x, y
		...
		// рассмотрим 2 ситуации: 1) машина движетс€ по Y	2) машина движетс€ по X
		...
		if (_direction == "up" || _direction == "down") {
			if ((Math::Abs(_yPos - (_nextPoint->Y - (TAXICAR_IMG_HEIGHT / 2))) < SPEED) && !isTurned) {
				...
				// (*) -> (3). создание новой точки, котора€ станет nextPoint2'ом
				// 1) генерируем перекрЄсток (*), пока не достигнем горизонтального движени€
		...
		// машина достигла X (двигалась по горизонтали)
			...
			if ((Math::Abs(_xPos - (_nextPoint->X - (TAXICAR_IMG_HEIGHT / 2))) < SPEED) && !isTurned) {
				...
				// задаЄм вертикальное движение до текущей (2)
	...
	void WayFind(Passenger^ passenger, array<array<Point^>^>^ Vertices) {
		...
		while (wayEndPoint != passenger->endPoint::get()) {
			...
			// добавить внутрь вайла проверку на самую маленькую разность совпадающих координат перекрЄстков путЄм генерации массива ¬—≈’ перекрЄстков одной координаты
